...
public class ProjectileController : MonoBehaviour
{
    ...
    private void Explode()
    {
        ...
        Collider[] colliders = Physics.OverlapSphere(transform.position, explosionRadius, collisionLayerMask);
        foreach (Collider col in colliders)
        {
            // Check if the collider belongs to an enemy
            // You may need to adjust the condition based on your enemy setup
            if (col.CompareTag("Enemy"))
            {
                // Assuming enemies have a script with a Die() method
                Enemy enemy = col.GetComponent<Enemy>();
                if (enemy != null)
                {
                    enemy.Die(); // Call a method to handle enemy death
                }
            }
        }
    }
